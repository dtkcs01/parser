...
def load_grammers():
    ...
    for non_terminal, obj in non_terminals.items():
        print(non_terminal, [ (first.symbol, _._LHS.symbol) for (first, _) in obj.first_set ], [ (first.symbol, _._LHS.symbol) for (first, _) in obj.follow_set ])

    Parser(terminals, non_terminals)
